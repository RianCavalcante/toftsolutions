...
export const Cover = ({ children, className }) => {
  ...
  return (
    <div
      ...
      ref={ref}
      className="relative hover:bg-white/10 group/cover inline-block bg-white/5 px-3 py-1 transition duration-200 rounded-lg"
    >
      <AnimatePresence>
        {hovered && (
          <motion.div
...
const CircleIcon = ({ className, delay }) => {
  return (
    <div
      className={cn(
        "pointer-events-none animate-pulse group-hover/cover:hidden h-1.5 w-1.5 rounded-full bg-white/30 opacity-50",
        className
      )}
    ></div>
  );
